...
export default function MinistriesSection() {
  return (
    <section className="py-24 bg-white">
      <div className="px-16 w-full max-w-[1280px] mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-[32px] font-bold text-[#081534] mb-4">Ministries for Everyone</h2>
          <p className="text-[16px] text-[#45464e] max-w-xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-6 h-auto md:h-[600px]">
          {ministries.map((m, i) => (
            <motion.div
              ...
              className={`${m.col} group relative overflow-hidden rounded-xl border border-[#c6c6cf] flex flex-col justify-end hover:border-[#081534] transition-all`}
            >
              ...
              <div className="relative z-10 p-6">
                <h3 className="text-[24px] font-semibold text-white">{m.name}</h3>
                <p className="text-[16px] text-white/80">{m.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
