...
// Fires a GA4 event via the global gtag set up by @next/third-parties/google's
// <GoogleAnalytics> component in the root layout. Safe no-op if analytics
// hasn't loaded yet (e.g. ad blockers, slow connections).
...
export default function GivingClient() {
  ...
  const handleCopy = (text: string, key: string, account: typeof localAccounts[number]) => {
    navigator.clipboard.writeText(text)
    setCopiedKey(key)
    setTimeout(() => setCopiedKey(null), 2000)

    // GA4 conversion event — tells us which giving channel actually gets used,
    // not just who visits the page.
            ...
            {localAccounts.map((acc, i) => (
              <motion.div key={acc.id}
                ...
                <CopyRow label="Account Number" value={acc.accountNo}
                  ...
                  onCopy={() => handleCopy(acc.accountNo, `local-${acc.id}`, acc)} />
              </motion.div>
            ))}
          </motion.div>
